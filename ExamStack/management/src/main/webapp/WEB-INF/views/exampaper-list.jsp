					...
					<div class="page-content">
						<div id="question-filter">
							<dl id="question-filter-pagetype">
								...
								<dd>
									<span data-id="0"
										<c:if test="${paperType == '0' }">
												 class="label label-info"
											</c:if>>全部</span>
									<c:forEach items="${fieldList }" var="item">
										<span data-id="${item.fieldId }"
											<c:if test="${paperType == item.fieldId }">
												 class="label label-info"
											</c:if>>${item.fieldName }</span>
									</c:forEach>
								</dd>
							</dl>
						...
						<div class="table-search table-controller-item"
							...
							<div class="input-group search-form">
								<input type="text" class="form-control" placeholder="搜索试卷"
								...
								<tbody>
									<c:forEach items="${paper }" var="item">
										<tr>
											...
											<td>${item.id }</td>
											<td><a
												href="<%=list[1]%>/exampaper/exampaper-preview/${item.id }"
												target="_blank" title="预览" class="td-paper-name">${item.name }</a></td>
											<td><span class="td-paper-duration">${item.duration}</span>分钟</td>
											<td><c:if test="${item.paper_type == '1' }">
													<span class="td-paper-type" data-id="${item.paper_type}">
														随机组卷 </span>
												</c:if> <c:if test="${item.paper_type == '2' }">
													<span class="td-paper-type" data-id="${item.paper_type}">
														模拟考试 </span>
												</c:if> <c:if test="${item.paper_type == '3' }">
													<span class="td-paper-type" data-id="${item.paper_type}">
														专家试卷 </span>
												</c:if></td>
											<td>${item.creator }</td>
											...
											<td><c:choose>
												...
												</c:choose> <a class="export-paper btn-sm btn-success"
												data-id="${item.id}" title="导出"><i
