...
<body>
			...
			<div class="row">
				...
				<div class="col-xs-10" id="right-content">
					...
					<div class="page-content">
						...
						<div id="exampaper-list">
							<div class="table-search table-controller-item"
								...
								<div class="input-group search-form"
									...
									<input type="text" class="form-control" placeholder="搜索学员"
							...
							<table class="table-striped table">
								...
								<tbody>
									<c:forEach items="${histList }" var="item">
										<tr>
											<td>${item.userName }${item.trueName } <br>
												准考证:${item.seriNo } <br> 身份证:${item.nationalId }
											</td>
											<%-- <td>${item.trueName }</td>
												<td>${item.seriNo }</td> --%>
											<td>${item.depName }</td>
											<td>${item.point } <br> <c:if
													test="${item.approved == 2 }">
													<a target="_blank"
														href="<%=list[1]%>/exam/mark-exampaper/${item.histId}">阅卷</a>
												</c:if> <c:if test="${item.approved == 3 }">
													<a target="_blank"
														href="<%=list[1]%>/exam/mark-exampaper/${item.histId}">重复阅卷</a>
												</c:if>
											</td>
											...
											<td><c:choose>
													<c:when test="${item.pointGet < item.passPoint }">
														<span style="color: #d9534f"> ${item.pointGet } </span>

													</c:when>
													<c:otherwise>
														<span style="color: #5cb85c"> ${item.pointGet } </span>
													</c:otherwise>
												</c:choose></td>
											<td><c:choose>
													<c:when test="${item.pointGet < item.passPoint }">
														<span style="color: #d9534f"> 未通过 </span>
											...
											<td><c:choose>
													<c:when test="${item.approved == 0 }">
														<button class="approved-btn btn btn-success"
															data-id="${item.histId }">通过</button>
														<button class="disapproved-btn btn btn-warning"
															data-id="${item.histId }">拒绝</button>
													</c:when>
												</c:choose> <c:if test="${item.approved == 3 }">
													<a target="_blank"
						...
						<div id="page-link-content">${pagingStr }</div>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- 管理用户组modal -->
	<div class="modal fade" id="link-user-group-modal" tabindex="-1"
		role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">
						关闭窗口</button>
					<button id="link-user-group-btn" type="button"
