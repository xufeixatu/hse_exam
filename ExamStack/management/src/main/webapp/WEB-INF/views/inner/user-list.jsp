...
<body>
	...
	<div id="question-list">
		<div class="table-controller">
			...
			<form id="form1" class="btn-group table-controller-item" action="reg.jsp" method="post" encType="multipart/form-data" style="float: left">
						<input onchange="change_input();" style="background:none;border:none"class="btn btn-default btn-sm" type="file"></input>
				</form>	
			<div class="btn-group table-controller-item" style="float: left">
			...
			<div class="table-search table-controller-item"
				...
				<div class="input-group search-form">
					<input type="text" class="form-control" placeholder="搜索用户"
						value="${searchStr }" id="txt-search"> <span
					...
					<tr>
						...
						<td><span class="r-name">${item.userName }</span>
							<div>
								...
								<a
									href="javascript:parent.open('admin/training/student-practice-status/${item.userId}','_blank' )">学习进度</a>
							</div></td>
						<td>
						...
						<td>
							<div class="r-company" style="display: none;">
								${item.company }</div>
							<div class="r-dept">${item.department }</div>
						</td>
						...
						<td><span class="r-update-btn btn-sm btn-success"
							...
							data-status="${!item.enabled }"> <c:choose>
									<c:when test="${!item.enabled }">
										启用
									</c:when>
									<c:otherwise>
						...
						</span> <span class="r-reset-pwd-btn btn-sm btn-warning" data-id="21321"
							data-depid="${item.depId }">密码</span> <c:choose>
								...
								<c:otherwise>
									<span class="unlink-user-r-btn btn-sm btn-danger" title="移出分组"
										data-id="${item.userId }" data-group="${groupId }"> <i
										class="fa fa-times-circle"></i>
									</span>
								</c:otherwise>
							</c:choose> <%-- <span class="link-user-r-btn simple-btn" title="关联用户">
													<i class="fa fa-random"></i>
								</span>
