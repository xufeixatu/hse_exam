							...
							<table class="table-striped table">
								<thead>
									<tr>
										...
										<td>状态</td>
										<c:if
											test="${fn:contains(sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.authorities,'ROLE_ADMIN') }">
											<td>操作</td>
										</c:if>
								...
								<tbody>
									<c:forEach items="${examList }" var="item">
										<tr>
											<td>${item.examId }</td>
											<td><span>${item.examName }</span> <br> <a
												href="<%=list[1]%>/exam/exam-student-list/${item.examId }"
												target="_blank">学员名单</a></td>
											...
											<c:if
												...
												<td><c:choose>
													...
													</c:choose> <span class="link-user-r-btn btn-sm btn-info" title="关联学员"
													data-id="${item.examId }"> <i class="fa fa-random"></i>
												</span></td>
											</c:if>
