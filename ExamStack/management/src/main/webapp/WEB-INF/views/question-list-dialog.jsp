...
<body>
							...
							<table class="table-striped table">
								...
								<tbody>
									...
									<c:forEach items="${questionList }" var="items">
										<tr>
											<td><input type="checkbox" value="${items.id }">
											</td>
											<td class="question-id">${items.id }</td>
											<td><a
												href="<%=list[1]%>/question/question-preview/${items.id }"
												target="_blank" title="预览">${items.name }</a> <span
												class="examing-point">${items.examingPoint} </span></td>
	...
	<script type="text/javascript"
		src="resources/js/question-list4dialog.js?v=1"></script>
</body>
</html>
